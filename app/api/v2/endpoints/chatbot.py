...
async def init_session(request: InitSessionRequest):
    ...
    try:
        ...
        data = await execute_sp("ia.sp_simBudFormSA_one", {"user_fk": config.USER_FK, "sa_fk": request.sa_fk, "form_fk": request.form_fk})
        json_string = data[0].get('EcrituresDetails')
        if json_string is None:
            context_data = pd.DataFrame()
            simple_dict = {}
            try:
                r.set(session_id, pickle.dumps([context_data, [], simple_dict]))
...
async def chat(request: ChatRequest):
    ...
    if res is None:
        ...
        return {
            "message": "Le tableau de donnée est vide.",
            "response_time": formatted_time
        }
    try:
        context_data, history_list, simple_dict = pickle.loads(res)
        history = list(history_list)
        
        if len(history) > config.HISTORY_LENGTH+1:
            history = [history[0]] + history[-config.HISTORY_LENGTH:]
        ...
        try:
            r.set(request.session_id, pickle.dumps([context_data, history, simple_dict]))
        ...
        raise  # Already logged higher up
    except Exception as e:
